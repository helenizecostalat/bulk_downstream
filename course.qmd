...
if (length(top_genes) > 0) {
  ...
  plot_tbl <- as.data.frame(t(deg_mat_sub[top_genes, , drop = FALSE])) %>%
    ...
    left_join(dig_meta %>% dplyr::select(sample, space), by = "sample") %>%
    left_join(gene_labels, by = "gene")

  p_digs <- ggplot(plot_tbl, aes(x = space, y = degree, fill = space)) +
    geom_boxplot(outlier.size = 0.6) +
    geom_jitter(width = 0.1, size = 0.8, alpha = 0.6) +
    facet_wrap(~ gene_label, scales = "free_y") +
    theme_bw() +
    labs(title = "Top DIGs: conectividade por grupo", x = NULL, y = "Grau")
...
conn_tbl <- data.frame(sample = names(conn_vec), connectivity = conn_vec) %>%
  left_join(dig_meta %>% select(sample, space), by = "sample")

ssgsea_sub <- ssgsea_mat[, conn_tbl$sample, drop = FALSE]

reg_results <- lapply(rownames(ssgsea_sub), function(gs) {
  df <- conn_tbl
  df$score <- as.numeric(ssgsea_sub[gs, ])
  fit <- lm(connectivity ~ score + space, data = df)
...
ogn_map <- gene_symbols %>% filter(SYMBOL == "OGN") %>% pull(ENSEMBL)
ogn_map <- ogn_map[!is.na(ogn_map)]

if (length(ogn_map) > 0 && ogn_map[1] %in% rownames(deg_mat_sub)) {
  ogn_conn <- as.numeric(deg_mat_sub[ogn_map[1], conn_tbl$sample])
  ...
  plot_ssgsea_tbl <- data.frame(
    sample = conn_tbl$sample,
    space = conn_tbl$space,
    ogn_conn = ogn_conn
  )

  plot_long <- lapply(top_sets, function(gs) {
    data.frame(
      sample = plot_ssgsea_tbl$sample,
      space = plot_ssgsea_tbl$space,
      ogn_conn = plot_ssgsea_tbl$ogn_conn,
      geneset = gs,
      ssgsea = as.numeric(ssgsea_sub[gs, plot_ssgsea_tbl$sample])
    )
  }) %>%
    bind_rows()
  ...
  stats_tbl <- plot_long %>%
    ...
    mutate(label = paste0("R2=", signif(r2, 2), "\n", "p=", signif(p_val, 2)))

  p_ssgsea <- ggplot(plot_long, aes(x = ogn_conn, y = ssgsea, color = space)) +
    geom_point(size = 2, alpha = 0.8) +
    ...
    theme_bw() +
    labs(title = "ssGSEA vs conectividade de OGN", x = "Conectividade OGN", y = "ssGSEA")
